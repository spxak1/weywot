...
6. If you already have Pop!_OS as your only OS and want to install Windows, you will need to **boot from USB** and start at **step 3** above. Note that if you're using Pop!_OS's built-in full disk encryption, you cannot resize the partition. In that case you will need to either reinstall the OS without encryption or purchase another disk to install Windows on.
