...
Then, navigate to the config files for your BT adapter, enter the adapter's folder, then enter's the device's folder
...
Also note, the MAC address ends up with **8c**. This is because some mice have a rolling (changing) MAC address, and everytime you pair them anew, the MAC address changes (the last bit anyway). 
