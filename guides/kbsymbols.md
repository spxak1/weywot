...
So to type the square root symbol, press compose and you see a _._ blinking. Release and type the recipe, ```s``` and ```r```. When ```r``` is released you see the √.
...
Instead you type ```ctrl+shift+U``` and the prompt turns to _u_ afte which (you release the keys and) you type the code, e.g. ```221a``` and space to see the √.
